...
mapboxgl.accessToken =
  "pk.eyJ1IjoiZnJhbmt6aHUxNjAiLCJhIjoiY205amN4cWt2MDk1MTJqcHM2ZmxseXE4cCJ9.vmdZkfIdPVYRkaRus1_IRg";
...
class InfoControl {
  onAdd(map) {
    ...
    this._container.className =
      "mapboxgl-ctrl legend-control legend-vertical-list info-control";
    this.collapsed = true;
...
const MapBox = forwardRef(
  ({ zoom = 14, style, height = "500px", onRouteDrawn }, ref) => {
    ...
    // Added For Reactive Mapbox
    const [mapSize, setMapSize] = useState(getFixedMapboxSize());
    ...
    // For the sizing of the map box
    useEffect(() => {
      const handleResize = () => setMapSize(getFixedMapboxSize());
    ...
    async function onDrawRoute(e) {
      ...
      matched.coordinates.forEach((_, i, arr) => {
        ...
        const dist =
          distance([lng1, lat1], [lng2, lat2], { units: "kilometers" }) * 1000;
        ...
        segments.push({
          ...
          properties: { angle, color: getColorForAngle(angle) },
        });
      });

      map
        .getSource("route-color")
        .setData({ type: "FeatureCollection", features: segments });

      const max = Math.max(
        ...segments.map((f) => Math.abs(f.properties.angle))
      );
    ...
    useImperativeHandle(ref, () => ({
      ...
      startDraw() {
        mapRef.current &&
          mapRef.current._controls
            .find((c) => c instanceof MapboxDraw)
      ...
      clearDraw() {
        ...
        mapRef.current
          .getSource("raw-route")
          .setData({ type: "LineString", coordinates: [] });
      },
